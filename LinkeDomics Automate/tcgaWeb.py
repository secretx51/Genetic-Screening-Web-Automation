...
MAIN_DIR = "/Users/trent/Developer/tcgabiolinks"
ERROR = 5
...
def tcga(gene):
    ...
    while True:
        try:
            ...
            driver.get_screenshot_as_file(f"{MAIN_DIR}/tcga_output.png")
            error = compareImages()
...
def compareImages():
    og_img = cv2.imread(f"{MAIN_DIR}/tcga_output_failed.png")
    new_img = cv2.imread(f"{MAIN_DIR}/tcga_output.png")
...
def writeErrors(errors: list):
    with open(f"{MAIN_DIR}/tcgaErrors.txt", 'w') as file:
            file.truncate(0)
            for error in errors:
                file.write(f'{error},')
    file.close()
    ...
    for gene in genes:
        try:
            if checkFileExists(f"{gene}.csv"):
                    continue
            df = tcga(gene) #obtain db webpage
...
def combineDownloads(genes):
    ...
    for gene in genes:
        try:
            ...
            concat_df.to_csv(f"{MAIN_DIR}/tcga_output.csv", index=False) #write concat df to csv
...
def countGenes(column):
    df = pd.read_csv(f"{MAIN_DIR}/tcga_output.csv")
...
def formatData(value):
    df_sorted = sortColumns(f"{MAIN_DIR}/tcga_output.csv")
    ...
    save_excel_sheet(output_df, f"{MAIN_DIR}/tcga_output_formatted.xlsx", value)

def main():
    genes = importFile(f"{MAIN_DIR}/tcgaGenes.txt")
    ...
    countGenes("Gene").to_csv(f"{MAIN_DIR}/tcga_counts.csv") #Output a count file
